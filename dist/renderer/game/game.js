...
MarginCont.prototype.resize = function (wi, hi) {
  ...
  for (var i = 0; i < this.items[0].length; i++) {
    ...
    it.x = this.MARGIN;
    if (i == 0) it.y = this.MARGIN;else it.y = pit.y + pit.height + this.MARGIN;
  }
  ...
  for (var i = 0; i < this.items[2].length; i++) {
    ...
    it.x = this.MARGIN;
    if (i == 0) it.y = ch - it.height - this.MARGIN;else it.y = pit.y - it.height - this.MARGIN;
  }
...
RectCont.prototype.resize = function (wi, hi) {
  this.w = wi;
  this.h = hi; //var min = Math.min(wi, hi);
...
BDFac.get = function (key) {
  if (BDFac._bds[key] == null) throw "No BitmapData with key \"" + key + "\"!";
  return BDFac._bds[key];
};
